 ...
 * item mode's output since it goes through the identical rasterizeItemModel engine. Either way this
 * is the one hook PreviewScene, MaterialList, and ExportButtons read, so they can never drift out
 * of sync with each other or with which mode is active.
 */
export function useFinalVoxelGrid(): VoxelGrid | null {
  ...
  }, [state.mode, state.itemVoxelGrid, state.structureVoxelGrid, state.mobVoxelGrid, state.matchedFaces, state.shape]);
}
